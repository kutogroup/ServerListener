...
//Handle 自定义path和函数的对应关系，注意这里的handle尽量使用小写函数，否则会重复注册
...
func (c *HTTP) Serve(addr string, timeout time.Duration) {
	...
	for i := 0; i < k.NumMethod(); i++ {
		...
		if f, ok := method.Func.Interface().(func(*HTTP, http.ResponseWriter, *http.Request)); ok {
			...
			c.mux.HandleFunc(handlePath, func(w http.ResponseWriter, r *http.Request) {
				f(c, w, r)
			})
		}
	}
	...
	c.Logger.E("http error=", srv.ListenAndServe())
}
